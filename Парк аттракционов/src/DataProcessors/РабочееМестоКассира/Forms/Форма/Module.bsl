...
Процедура ЗаполнитьДекорациюБаллыКлиента()
	...
	МаксимумСписанияБаллов = Мин(ОстатокБаллов, СуммаИтого * МаксимальнаяДоля / 100);
			
	Шаблон = "Накоплено %1 баллов, из них можно списать %2. ";
	Описание = СтрШаблон(Шаблон, ОстатокБаллов, МаксимумСписанияБаллов);
	
	ШаблонСсылки = "#Заполнить_%1";
	СсылкаЗаполненияБаллов = СтрШаблон(ШаблонСсылки, XMLСтрока(МаксимумСписанияБаллов));
